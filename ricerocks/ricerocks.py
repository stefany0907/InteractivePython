...
class Sprite:
    ...
    def draw(self, canvas): 
        global dtime
        if self.animated:           
            explosion_index = dtime % 24
            src_center = explosion_info.get_center()
            src_size = explosion_info.get_size()
            canvas.draw_image(explosion_image, 
                    [(src_center[0] + explosion_index * src_size[0]), src_center[1]], src_size, self.pos, self.image_size)
            dtime += 1
        else:
...
def rock_spawner():
    ...
    if len(rock_group) < 12 and started:        
        ...
        rock_vel = [random.random() * 2 - .3, random.random() * 2 - .3]
...
def group_collide(group, other_object):  
    ...
    for inst in copy_group:        
        if inst.collide(other_object):
            remove_set.add(inst)
            flag = True
            a_explosion = Sprite(inst.get_position(), [0, 0], 0, 0, explosion_image, explosion_info)
            print inst.get_position()
